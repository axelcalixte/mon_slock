...
static int readescapedint(const char *str, int *i) {
  int n = 0;
  if (str[*i])
    ++*i;
...
static void writemessage(Display *dpy, Window win, int screen) {
  int len, line_len, width, height, s_width, s_height, i, k, tab_size, r, g, b,
      escaped_int, curr_line_len;
  XGCValues gr_values;
  ...
  for (i = k = 0; i < len; i++) {
    ...
    case 0x1b:
      ...
      break;
    default:
      XDrawString(dpy, win, gc, width + line_len, height + 20 * k, message + i,
                  1);
...
static void readpw(Display *dpy, struct xrandr *rr, struct lock **locks,
  ...
  while (running && !XNextEvent(dpy, &ev)) {
    running =
        !((time(NULL) - locktime < timetocancel) && (ev.type == MotionNotify));
    if (ev.type == KeyPress) {
...
static struct lock *lockscreen(Display *dpy, struct xrandr *rr, int screen) {
  ...
  for (i = 0, ptgrab = kbgrab = -1; i < 6; i++) {
    ...
    if (ptgrab == GrabSuccess && kbgrab == GrabSuccess) {
      ...
      XChangeProperty(
          dpy, lock->win, XInternAtom(dpy, "_NET_WM_WINDOW_OPACITY", False),
